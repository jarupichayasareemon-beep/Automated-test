...
from .environments import apply_environment_profile
...
def run(event_url: str | None = None) -> int:
    # ต้องเช็ค/sync .env ตาม environment ของลิงก์ (prod/dev) ก่อน import Config หรือ module
    # ใดๆ ที่ import Config เพราะ Config อ่านค่าจาก env ตอน import ครั้งแรกเท่านั้น — ถ้า import
    # ไปแล้วก่อนเช็ค env จะได้ค่าเก่าค้างอยู่ (เช่น login AWS account ผิดใบ)
    if event_url:
        applied = apply_environment_profile(event_url)
        if applied:
            log.info(
                "Detected environment: %s (%s) -> synced .env (AWS_CONSOLE_ACCOUNT_ALIAS=%s)",
                applied["name"], applied["host"], applied["AWS_CONSOLE_ACCOUNT_ALIAS"],
            )
